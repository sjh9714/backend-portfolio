...
export const finmate: Project = {
  ...
  claimBoundary: [
    "하나금융그룹 × 금융감독원 2026 청년 금융인재 프로젝트의 팀 프로젝트(가가제작소)입니다. 위 내용은 제가 작성한 프론트엔드(finmate-app, 365커밋 중 361커밋)와 백엔드(finmate-api, 커밋 100%)의 범위입니다. 원래 백엔드는 프론트와 다른 제품 정의로 설계돼 앱에 연결된 적이 없어, 앱의 데이터 계층을 계약으로 삼아 다시 구현했습니다.",
    "기획·리서치와 합성 데이터셋(finmate-data)은 팀원의 작업이라 제 성과로 적지 않았습니다. 발표자료에 실린 카드 데이터 EDA 규모와 설문 표본 수치도 같은 이유로 싣지 않습니다.",
    "성능 수치는 Apple M4 로컬의 Testcontainers Postgres에서 단일 클라이언트로 잰 값이며 운영 성능이나 SLO 주장이 아닙니다. Postgres 설정은 컨테이너 기본값이고 튜닝하지 않았습니다.",
