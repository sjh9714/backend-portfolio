...
export const realtimeChat: Project = {
  ...
  claimBoundary:
    "조회 부하 수치는 2026-08-06에 현재 커밋(9663f58)에서 다시 측정한 값입니다. Apple M4 단일 머신의 로컬 Docker에서 애플리케이션·PostgreSQL·Redis·Kafka를 함께 실행했고, 단일 인스턴스 기준이며 JVM warmup을 두지 않았습니다. 운영 성능이나 SLO 주장이 아닙니다. WebSocket 전달 완전성은 2026-08-08에 현재 커밋(258b837)에서 3회 다시 쟀습니다. 다만 부하를 거는 쪽까지 같은 노트북에서 도는 로컬 반복이라, 여기서 확인한 것은 빠짐·중복·순서 위반이 없다는 사실뿐이고 지연 수치는 근거 문서에만 남겼습니다. 전후 비교는 최적화 직전 커밋(787781c)과 현재 커밋을 2026-08-08에 같은 부하 도구로 연달아 잰 값입니다. 그 사이에는 쿼리 구조뿐 아니라 인덱스 5개와 Redis 캐시도 함께 들어갔으므로, N+1 수정 하나의 몫이 아니라 그 최적화 작업 전체의 전후입니다. 그 200 VU 절대 수치는 앱을 Docker로 띄운 2026-08-06 실행과 기동 방식이 달라 서로 비교할 수 없습니다 — 전후 비교는 나란히 잰 근거 문서 안에서만 유효합니다.",
};
