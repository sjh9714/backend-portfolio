...
근거: `realtime-chat` 커밋 `6d69705` · `docs/evidence/NPLUS1_BEFORE_AFTER_2026-08-08.md`
...
| | before | after |
|---|---:|---:|
| median | 9.94ms · 9.12ms | **1.82ms · 1.83ms** |
...
| | before | after |
|---|---:|---:|
| RPS | 173.0 · 195.5 | **244.0 · 254.7** |
| p95 | 620.95ms · 619.73ms | 691.75ms · 654.1ms |
| HTTP 실패 | 0 | 0 |

사이트에는 반올림해 **median 9.9ms → 1.8ms**, **처리량 41 → 65 반복/초**로 싣는다.

**포화 구간의 p95는 개선 지표가 아니다.** 병목을 없앤 만큼 더 받아들이고 큐가 길어진다.
사이트도 그렇게 적었다.
